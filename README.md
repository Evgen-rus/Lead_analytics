...
## Если включён локальный VPN/туннель (Happ)

Обычный `npm run dev` может слушать `localhost`, и браузер уйдёт в туннель. Запускай фронт так:
...
Открывать только так: http://127.0.0.1:5174
...
После формирования аналитики web-интерфейс может перенести все листы готового Excel-отчёта
в одну настроенную Google Таблицу. Для каждого листа создаётся новая вкладка с префиксом
`проект — дата анализа`; существующие вкладки не перезаписываются.

Создайте `.env` в корне проекта:

```env
GOOGLE_CREDENTIALS_FILE=credentials/service-account.json
GOOGLE_SPREADSHEET_ID=идентификатор_таблицы
```

Целевая таблица должна быть доступна service account на редактирование. `.env` и папка
